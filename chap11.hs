...
mapTree2 _ Leaf = Leaf
mapTree2 f (Node left a right) = 
  Node (mapTree2 f left) (f a) (mapTree2 f right)
testTree =
  Node (Node Leaf 3 Leaf) 1 (Node Leaf 4 Leaf)
mapExpected =
  Node (Node Leaf 4 Leaf) 2 (Node Leaf 5 Leaf)
mapOkay =
  if mapTree2 (+1) testTree == mapExpected
  then print "yup okay!"
  else error "test failed!"
inorder Leaf = []
inorder (Node left a right) = (inorder left) ++ a 
  : inorder right
foldTree :: (a -> b -> b)
  -> b
  -> BinaryTree a
...
foldTree3 :: (a -> b -> b)
  ...
  -> BinaryTree a
  -> b
foldTree3 f ac bt = foldr f ac (inorder bt)
checkFT = foldTree (:) [] testTree

-- -- Equational Reasoning:
-- check = foldTree (:) [] 
--   (Node (Node Leaf 3 Leaf) 1 (Node Leaf 4 Leaf))
-- check2 = (foldTree (:) (((:) 1) (foldTree (:) [] 
--   (Node Leaf 4 Leaf))) (Node Leaf 3 Leaf))
-- check3 = (foldTree (:) (((:) 1) (foldTree (:) (((:) 4) 
--   (foldTree (:) [] Leaf)) Leaf)) (Node Leaf 3 Leaf))
-- check4 = (foldTree (:) (((:) 1) (foldTree (:) (((:) 4) 
--   []) Leaf)) (Node Leaf 3 Leaf))
-- check5 = (foldTree (:) (((:) 1) (foldTree (:) [4] Leaf)) 
--   (Node Leaf 3 Leaf))
-- check6 = (foldTree (:) (((:) 1) [4]) (Node Leaf 3 Leaf))
-- check7 = foldTree (:) [1,4] (Node Leaf 3 Leaf)
-- check8 = foldTree (:) (((:) 3) (foldTree (:) [1,4] Leaf)) Leaf
-- check9 = foldTree (:) (((:) 3) [1,4]) Leaf
-- = [3,1,4]
